...
class LineWithTargetPlot(BasePlot):
    def __init__(self, name, data_x, plot_data, target_data, x_name, y_name):
        super().__init__(name)
        self.data_x = data_x
    ...
    def draw_plot(self, axs):
        ...
        axs.plot(self.data_x, self.target_data, 'k--')

        max_y = max(max(self.plot_data), max(self.target_data))
        min_y = min(min(self.plot_data), min(self.target_data))
